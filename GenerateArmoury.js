...
function InitFolders() {
    ...
    const directory = `${ProjectDir}/autogenerated/variantmeshes/variantmeshdefinitions/auto/`;
...
function GenerateBasicArmourySetIds() {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        result.push(`A.${row.FaceId}.${row.HelmetId}.${row.ArmourId}.${row.WeaponId}.${row.ShieldId}`);
    }
    return result
}
            ...
            for (const armour of armours) {
                ...
                for (let shield of shields) {
                    ...
                    for (const weapon of weapons) {
                        ...
                        const x = `A.${face}.${helmet}.${armour}.${weapon}.${shield}`
...
function GenerateXMLFromIds(xmlname) {
    ...
    for (const combinationId of combinationsIds) {
        const faceId = combinationId.split(".").slice(0, 2).join(".")
        const basicId = basicIds.find( id => id.includes(faceId) )
        const faceIdOnly = combinationId.split(".")[1]
...
function GenerateCampaignCharacterArtSetsTables(tableName) {
    ...
    for (const combinationId of combinationsIds) {
        const faceId = combinationId.split(".")[1]
        const index = basicArmourSet.FaceId.indexOf(faceId)
    ...
    for (let i = 0; i < entries.length; i += CHUNK_SIZE) {        
        const theTableName = `autogenerated_${tableName}_${i}_chunk`
        let   header = `art_set_id	agent_type	culture	subculture	faction	is_custom	is_male	agent_subtype	campaign_map_scale\n`
        header+= `#campaign_character_art_sets_tables;6;db/campaign_character_art_sets_tables/${theTableName}								`
        const chunk = entries.slice(i, i + CHUNK_SIZE);
        const tableRow = chunk.join("\n")
        const tsv = `${header}\n${tableRow}`.trim()
        fs.writeFileSync(`${ProjectDir}/autogenerated/db/campaign_character_art_sets_tables/${theTableName}.tsv`, tsv)
    }    
}
...
function GenerateUniforms(tableName) {
    ...
    for (let i = 0; i < entries.length; i += CHUNK_SIZE) {        
        ...
        header += `#agent_uniforms_tables;10;db/agent_uniforms_tables/${theTableName}	`
        const chunk = entries.slice(i, i + CHUNK_SIZE);
        const tableRow = chunk.join("\n")
        const tsv = `${header}\n${tableRow}`.trim()
        fs.writeFileSync(`${ProjectDir}/autogenerated/db/agent_uniforms_tables/${theTableName}.tsv`, tsv)
    }    
}
...
function GenerateVariantTables(tableName, scale, mountScale) {
    ...
    for (const id of combinedIds) {
        ...
        entries.push(`${id}\t\tauto/${id}\t\t${mountScale}\t${scale}\t0\t`)
    }
    ...
    for (let i = 0; i < entries.length; i += CHUNK_SIZE) {        
        ...
        header += `#variants_tables;6;db/variants_tables/${theTableName}							`
        const chunk = entries.slice(i, i + CHUNK_SIZE);
        const tableRow = chunk.join("\n")
        const tsv = `${header}\n${tableRow}`.trim()
        fs.writeFileSync(`${ProjectDir}/autogenerated/db/variants_tables/${theTableName}.tsv`, tsv)
    }    
}
...
function GenerateCampaignCharacterArtsTables(tableName, seedNumber) {
    ...
    for (const id of combinedIds) {
        ...
        entries.push(`${(crc32(id) + seedNumber)}\t${id}\t0\t0\t\tnone\t${id}\t\t\t${animation.SeaUniform}\t${animation.NavyUniform}\t${animation.LandAnimation}\t${animation.SeaAnimation}\t${animation.NavyAnimation}\t0\t0\t0`)
        seedNumber++
    }   
    ...
    for (let i = 0; i < entries.length; i += CHUNK_SIZE) {        
        ...
        header += `#campaign_character_arts_tables;0;db/campaign_character_arts_tables/${theTableName}																`
        const chunk = entries.slice(i, i + CHUNK_SIZE);
        const tableRow = chunk.join("\n")
        const tsv = `${header}\n${tableRow}`.trim()
        fs.writeFileSync(`${ProjectDir}/autogenerated/db/campaign_character_arts_tables/${theTableName}.tsv`, tsv)
    }    
}
...
<VARIANT_MESH>
    ...
    <SLOT name="weapon_5" attach_point="be_prop_5" />
        ...
        `
        fs.writeFileSync(`${ProjectDir}/autogenerated/variantmeshes/variantmeshdefinitions/auto/${id}.variantmeshdefinition`, entry)
        counter++
...
function BuildPack(projectDir) {
    dirname = projectDir.replace(/\\/g, '/')
    ...
    try {
        fs.rmSync(`build/@autogenerated_${projectDir}.pack`)
    } catch (error) {}

    console.log(`build/@autogenerated_${projectDir}.pack creating!`)
    spawnSync(`${RPFM_PATH}/rpfm_cli.exe`, [
        '--game', 'warhammer_3', 
        'pack', 
        `create` , `--pack-path` , `build/@autogenerated_${projectDir}.pack`,   
    ], {
        stdio: "inherit",
        encoding: 'utf8'
    })
...
//BuildPack(ProjectDir, ProjectName)
